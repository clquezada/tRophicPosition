...
`tRophicPosition`, is an R package incorporating a Bayesian model for the calculation of consumer trophic position using stable isotopes with one or two baselines. As of 2017-05-22, the current version of the package is 0.6.8-8. `tRophicPosition` uses the powerful approach of Markov Chain Monte Carlo simulations provided by [JAGS](http://mcmc-jags.sourceforge.net) and the statistical language [R](https://www.r-project.org). A vignette with basics of the Bayesian model and a guide to the use of `tRophicPosition` can be found [here](https://github.com/clquezada/tRophicPosition/raw/master/A_guide_to_the_use_of_tRophicPosition_2016-06-12.pdf). Other vignettes can be browsed with `browseVignettes("tRophicPosition")`.
