...
trophicDensityPlot <- function (df = NULL, quantiles = FALSE, grouped = TRUE) {
  ...
  if (quantiles) {
    ...
    q <- list(ggplot2::geom_vline(aes(xintercept = TP.median), data = Species.stats,
                             linetype="dotted", color = "black", size=1),
              ggplot2::geom_vline(aes(xintercept = TP.025), data = Species.stats,
                                  color = "black", size=1),
              ggplot2::geom_vline(aes(xintercept = TP.975), data = Species.stats,
                                  color = "black", size=1),
              ggplot2::geom_vline(aes(xintercept = TP.25), data = Species.stats,
                                  color = "grey", size=1),
              ggplot2::geom_vline(aes(xintercept = TP.75), data = Species.stats,
                                  color = "grey", size=1))
  }
